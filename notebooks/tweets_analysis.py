# -*- coding: utf-8 -*-
"""Copy of CETSP tweets analysis

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1HPk91kX325m9FhawyakSUitcV01Lhtvm
...
from notebooks.auth_config import google_geolocation_key
from notebooks.config import location_pattern, location_pattern_exception, address_pre_patterns, \
...
def load_tweets():
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name('dissertacao-5bc4c6d93f19.json', scope)
    gc = gspread.authorize(credentials)

    rows = []
    for account_id in config.ids:
        try:
...
"""# Tokens and stopwords processing"""
...
clf_tfidf = LogisticRegression(C=30.0, class_weight='balanced', solver='newton-cg', multi_class='multinomial',
                               n_jobs=cores, random_state=40)
clf_tfidf.fit(X_train_tfidf, y_train)
y_predicted_logistic_regression_clf = clf_tfidf.predict(X_test_tfidf)
...
decision_tree_clf = decision_tree_clf.fit(X_train_tfidf, y_train)
...
gnb = GaussianNB()
gnb_y_pred = gnb.fit(X_train_tfidf.toarray(), y_train).predict(X_test_tfidf.toarray())

print("Gaussian Naive Bayes: accuracy = %.3f, precision = %.3f, recall = %.3f, f1 = %.3f" % (
    get_metrics(y_test, gnb_y_pred)))


svm_clf = OneVsRestClassifier(SVC(kernel='linear', probability=True, class_weight=None), n_jobs=cores)
y_predicted_svm_clf = svm_clf.fit(X_train_tfidf, y_train).predict(X_test_tfidf)
...
importance_tfidf = get_most_important_features(tfidf_vec, clf_tfidf, 10)
